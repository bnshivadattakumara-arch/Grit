...
export const WorldMapTerminal: React.FC<{ onCountryClick: (id: string) => void }> = ({ onCountryClick }) => {
  ...
  const sortedTickers = useMemo(() => {
    return (Object.values(liveData) as CountryData[]).sort((a,b) => {
      const priority = ['IND', 'USA', 'CHN', 'RUS', 'ISR'];
                   ...
                   >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className={`text-[11px] font-bold uppercase truncate w-32 ${['IND', 'ISR'].includes(item.id) ? 'text-terminal-green' : 'text-white'}`}>{item.name}</span>
                        </div>
                      </div>
