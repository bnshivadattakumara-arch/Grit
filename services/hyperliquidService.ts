...
export const fetchHyperliquidTickers = async (): Promise<EnrichedTicker[]> => {
    ...
    // Fallback if structured data fails but allMids works
    if (tickers.length === 0 && allMids) {
        Object.entries(allMids).forEach(([name, price]) => {
            tickers.push({
                symbol: name,
                baseAsset: name,
                quoteAsset: 'USDC',
                lastPrice: price as string,
                ...
                spread: 0,
                priceChange: '0',
                weightedAvgPrice: price as string,
                prevClosePrice: price as string,
                lastQty: '0',
                bidPrice: price as string,
                bidQty: '0',
                askPrice: price as string,
                askQty: '0',
                openPrice: price as string,
                openTime: 0,
                closeTime: Date.now(),
                firstId: 0,
                lastId: 0,
